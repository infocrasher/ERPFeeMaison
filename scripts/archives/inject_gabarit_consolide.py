#!/usr/bin/env python3
"""
Script d'injection parfait pour le gabarit consolidé
Utilise les ingrédients similaires fusionnés avec prix moyen pondéré
"""
...
def load_gabarit_data():
    """Charge les données du gabarit consolidé"""
    try:
        logger.info("📂 Chargement du gabarit consolidé...")
        
        # Charger toutes les feuilles
        ingredients = pd.read_excel('Gabarit_Consolide.xlsx', sheet_name='Ingrédients')
        products = pd.read_excel('Gabarit_Consolide.xlsx', sheet_name='Produits_Finis')
        recipes = pd.read_excel('Gabarit_Consolide.xlsx', sheet_name='Recettes')
        ingredients_recipe = pd.read_excel('Gabarit_Consolide.xlsx', sheet_name='Ingrédients_Recette')
...
def main():
    ...
    with app.app_context():
        logger.info("🚀 Début de l'injection du gabarit consolidé")
